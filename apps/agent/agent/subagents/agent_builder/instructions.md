...
so ask one `ask_question` that offers both. When the answer is that the channel
exists, tell the user to add the Slack bot to it, ask them to answer again after
that is done, and re-inspect when they answer. When the answer is that the
channel does not exist, call `create_slack_channel`. Never send the user to
Slack to make a channel, and never reach for `run-channel`, which resolves to
the channel *that run* opened and so gives one channel per event. A person
