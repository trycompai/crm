...
- **Lifecycle specialists are Deploy-gated team agents.** Optional manifest field
  ...
  template lives in `apps/agent/agent/lib/lifecycle-qualify.ts`. The Advance template
  lives in `apps/agent/agent/lib/lifecycle-advance.ts`. Both are recommend-only: CRM
  note/task plus `run.summary`, no send tools, and Advance never mutates deal stage.
  Saving never makes a version LIVE.
