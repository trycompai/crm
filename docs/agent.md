...
- **Lifecycle specialists are Deploy-gated team agents.** Optional manifest field
  ...
  template lives in `apps/agent/agent/lib/lifecycle-qualify.ts`. The Close template
  lives in `apps/agent/agent/lib/lifecycle-close.ts`. Both are recommend-only: CRM
  note/task plus `run.summary`, no send tools. Close never reopens deals or writes
  finance fields. Saving never makes a version LIVE.
